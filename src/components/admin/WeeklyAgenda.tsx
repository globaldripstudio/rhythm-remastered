...
import { format, startOfWeek, endOfWeek, eachDayOfInterval, addWeeks, subWeeks, isSameDay, addHours, setHours, setMinutes } from 'date-fns';
...
const HOUR_HEIGHT = 40; // Reduced to fit on screen
const STUDIO_OPEN = 10;
...
const isClosedHour = (hour: number, day: Date) => {
  if (isWeekend(day)) return true;
  return hour < STUDIO_OPEN || hour >= STUDIO_CLOSE;
};

const isLunchHour = (hour: number, day: Date) => {
...
const WeeklyAgenda = () => {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    setSelection({
      day: dragStart.day,
      startHour: Math.floor(Math.min(dragStart.hour, currentHour)),
      endHour: Math.ceil(Math.max(dragStart.hour, currentHour))
    });
  }, [isDragging, dragStart]);

  const handleMouseUp = () => {
    if (isDragging && selection) openDialogForSelection();
    setIsDragging(false);
    setDragStart(null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingEvent) {
      ...
      else { toast({ title: "Événement modifié" }); fetchEvents(); }
    } else {
      const { error } = await supabase.from('events').insert(eventData);
      if (error) toast({ title: "Erreur", description: error.message, variant: "destructive" });
      else { toast({ title: "Événement créé" }); fetchEvents(); }
    }
  ...
  const handleDelete = async (eventId: string) => {
    ...
    if (error) toast({ title: "Erreur", description: error.message, variant: "destructive" });
    else { toast({ title: "Événement supprimé" }); fetchEvents(); }
  };

  const getEventsForDay = (day: Date) => events.filter(event => isSameDay(new Date(event.start_time), day));
  ...
  const today = (day: Date) => isSameDay(day, new Date());
  ...
  return (
    <Card className="border-primary/20">
      ...
      <CardContent className="p-0">
        ...
        <div className="grid grid-cols-[40px_repeat(7,1fr)] border-b border-border">
          ...
          {days.map(day => (
            <div 
              key={day.toISOString()} 
              className={cn(
                "p-1.5 text-center border-l border-border",
                today(day) && "bg-primary/10",
        ...
        >
          <div className="grid grid-cols-[40px_repeat(7,1fr)]" style={{ height: HOURS.length * HOUR_HEIGHT }}>
            {/* Hour Labels */}
            <div className="relative">
              {HOURS.map((hour, idx) => (
                <div 
                  key={hour}
                  className="absolute w-full text-[10px] text-muted-foreground text-right pr-1 -translate-y-1/2"
            ...
            {days.map(day => {
              ...
              return (
                <div key={day.toISOString()} className={cn("relative border-l border-border", isWeekend(day) && "bg-muted/20")}>
                  {/* Hour lines with backgrounds */}
                  {HOURS.map((hour, idx) => (
                    <div
                      key={hour}
                      className={cn(
                        "absolute w-full border-t border-border/40",
                        isClosedHour(hour, day) && "bg-muted/50",
                        isLunchHour(hour, day) && "bg-secondary/10"
                      )}
                  ...
                  {selection && isSameDay(selection.day, day) && (
                    <div
                      className="absolute left-0.5 right-0.5 bg-primary/30 border-2 border-primary border-dashed rounded z-10"
                      style={{
                  ...
                  {dayEvents.map(event => {
                    ...
                    return (
                      <div
                        ...
                        className="absolute left-0.5 right-0.5 rounded p-1 cursor-pointer overflow-hidden hover:opacity-90 transition-opacity z-20"
                        style={{
                          top: pos.top,
                          height: pos.height,
                      ...
                      >
                        <div className="text-[10px] font-medium truncate leading-tight">{event.title}</div>
                        {pos.height > 30 && event.description && (
                          <div className="text-[9px] text-muted-foreground truncate">{event.description}</div>
                        )}
                      </div>
                    );
                  })}
